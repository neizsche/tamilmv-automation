const { qbitClient } = require('../utils/httpClient');
const fs = require('fs');
...
class QBittorrentClient {
    ...
    _createAddTorrentForm(torrentFile) {
    ...
    async addTorrent(torrentFile) {
        ...
        const form = this._createAddTorrentForm(torrentFile);

        try {
            await qbitClient.post(`${config.TORRENT_URL}/add`, form, {
                headers: {
                    ...form.getHeaders(),
                    'User-Agent': 'Fiddler',
                    Cookie: `SID=${sid}`,
            ...
            if (error.response?.status === 403) {
                ...
                try {
                    ...
                    const retryForm = this._createAddTorrentForm(torrentFile);

                    await qbitClient.post(`${config.TORRENT_URL}/add`, retryForm, {
                        headers: {
    ...
    async getTorrents(applyFilter = false) {
        ...
        try {
            const { data } = await qbitClient.get(`${config.TORRENT_URL}/info`, {
    ...
    async manageTorrents(torrents, action) {
        ...
        try {
            await qbitClient.post(`${config.TORRENT_URL}/${action}`, form, {
                headers: {
                    ...form.getHeaders(),
                    Cookie: `SID=${sid}`,
                }
    ...
    async cleanupCompletedTorrents() {
        try {
            ...
            // Stop torrents that are 100% downloaded but still uploading
            const fullyDownloaded = allTorrents.filter(
                (t) =>
                    t.tags === config.QBITTORRENT.TAG &&
                    t.progress === 1 &&
                    t.state !== 'stoppedDL' &&
                    t.state !== 'stoppedUP'
            );
            if (fullyDownloaded.length > 0) {
                await this.manageTorrents(fullyDownloaded, 'stop');
            }

            // Delete stalled, errored, or missing file torrents
            const stalled = allTorrents.filter(
                (t) =>
                    t.tags === config.QBITTORRENT.TAG &&
                    t.category === config.QBITTORRENT.CATEGORY_ACTIVE &&
                    (t.state === 'stalledDL' || t.state === 'error' || t.state === 'missingFiles')
            );
            if (stalled.length > 0) {
                await this.manageTorrents(stalled, 'delete');
            }
        } catch (error) {
